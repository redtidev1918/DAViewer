...
- 设置里新增「账号 Cookie」：可以查看当前 DeviantArt 网站会话 Cookie，一键复制导出为格式化 JSON；也可以把别处导出的 Cookie 粘贴导入，直接恢复登录会话，支持导出的 JSON、浏览器插件导出的 Cookie 列表和 `name=value; …` 三种格式。
- 导入只接受与当前账号一致的会话：切换账号需先退出登录，应用绝不会把两个不同账号的会话混在一起；无法识别或已失效的 Cookie 会被拒绝并自动恢复导入前的状态。
- 仅靠 Cookie 导入恢复网页会话后，如果还没完成官方授权登录，会引导你登录一次，让关注、通知、收藏和原图下载等功能可用（可以跳过，推荐流不受影响）。
...
## 0.2.181（下载按钮精简与保存位置提示）

### Changed

- 详情页下载入口从大按钮改为轻量图标按钮，减少对作品内容的视觉干扰；保留
  48dp 触控区、Tooltip、禁用原因与进度圈。
- 图片保存到系统下载目录后，弹出包含实际保存位置的完成提示，替换原来只有
  “已下载”的通用文案。

## 0.2.180（全屏图片单张下载与文档站点）

### Added

- 全屏查看图片时长按当前图片即可单独下载；多图作品可逐张选择要下载的页面，
  与整组原图下载相互独立。
- 新增 docsify 文档站点（docs/ 下的架构、认证、网络、构建与 Web 适配说明，
  Markdown 单一来源），根 README 增加文档站点入口徽章。

### Fixed

- 重复长按同一张图片不再重复排队：已有下载会显示其当前状态，失败或已取消的
  任务可直接重新下载。
